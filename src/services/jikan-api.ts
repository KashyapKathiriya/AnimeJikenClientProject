...
export const animeService = {
    ...
    searchAnime: async (query: string, orderBy?: string, sort?: string): Promise<SearchResponse> => {
        const safequery = encodeURIComponent(query);
        if (orderBy) {
            return fetchData<SearchResponse>(`/anime?q=${safequery}&order_by=${orderBy}&sort=${sort}`);
        } else {
